...
function spawnFragments(atZ: number, side: number) {
  for (let i = 0; i < 5; i++) {
...
function updateFragments(delta: number) {
...
export function updateJoustAnimation(delta: number) {
  ...
  if (joustState === "charge") {
    ...
    if (resultAction!.type === "unhorse") {
      const loserModel = resultAction!.loserModel === "A" ? aModel : bModel;
      if (loserModel) {
        const tiltAngle = progress * (Math.PI / 2);
        loserModel.userData.rider.rotation.x = tiltAngle;
        loserModel.userData.rider.position.y = -progress * 0.8;
      }
    }
