...
export function renderUI() {
  ...
  for (const k of gameState.roster) {
    ...
    card.className = "knight-card" + (k.id === gameState.selectedKnightId ? " selected" : "");
    card.innerHTML = `
    ...
    card.addEventListener("click", () => {
      gameState.selectedKnightId = k.id;
      renderUI();
      if (joustState === "idle" && gameState.currentView === "tiltyard") {
        placeKnights(k, gameState.currentOpponent!);
      }
    });
