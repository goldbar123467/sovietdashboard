...
import { cloneKnightModel } from './loader';
import { colorizeGltfKnight } from './knightAnim';
...
/**
 * Helper: make a material fully transparent.
 * The mesh stays in the scene for raycasting / physics but is invisible.
 */
function ghostMaterial(mat: THREE.MeshStandardMaterial): THREE.MeshStandardMaterial {
...
export function createKnight3D(primaryColor: number, accentColor: number): KnightGroup {
  ...
  const useGltf = cloneKnightModel() !== null;

  // Choose material constructor — ghost if GLTF covers visuals
  const mat = (opts: THREE.MeshStandardMaterialParameters) => {
    const m = new THREE.MeshStandardMaterial(opts);
    return useGltf ? ghostMaterial(m) : m;
  };

  const bodyMat = mat({ color: 0x6b4a2a, roughness: 0.7 });
  const armorMat = mat({ color: primaryColor, roughness: 0.4, metalness: 0.2 });
  const accentMaterial = mat({ color: accentColor, roughness: 0.3, metalness: 0.3 });
  const darkMat = mat({ color: 0x1a1a1a, roughness: 0.8 });
  ...
  // ── GLTF overlay ──
  if (useGltf) {
    const gltf = cloneKnightModel()!;
    colorizeGltfKnight(gltf, primaryColor, accentColor);
    group.add(gltf);
    group.userData.gltfModel = gltf;
  }
